    ...
    <main className="min-h-screen bg-background-primary">
      ...
      <div className="relative">
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-16 md:py-24 z-10">
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/lists"
              className="inline-flex items-center gap-2.5 px-6 py-3 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-text-primary font-semibold transition-all"
            >
      ...
      {showCreateChooser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowCreateChooser(false)} />
          <div className="relative w-full max-w-lg rounded-2xl border border-white/10 bg-card p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-text-primary">Create</h2>
              <button onClick={() => setShowCreateChooser(false)} className="p-2 rounded-full hover:bg-white/10">
            ...
            <div className="space-y-5">
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-text-muted mb-2">Lists</p>
                <div className="grid gap-2">
                  {[
                    ...
                    <button
                      key={value}
                      onClick={() => router.push(`/lists?createType=${value}`)}
                      className="w-full rounded-xl border border-white/10 bg-background-primary px-4 py-3 text-left text-text-primary hover:border-accent/40 hover:bg-accent/5"
                    >
                      {label}
                    </button>
                  ))}
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.18em] text-text-muted mb-2">Profile Saves</p>
                <div className="grid gap-2">
                  {[
                    ["receipt", "Add Receipt"],
                    ["playerStatSnapshot", "Save Player Stats"],
                  ].map(([value, label]) => (
                    <button
                      key={value}
                      onClick={() => router.push(`/profile?createSave=${value}`)}
                      className="w-full rounded-xl border border-white/10 bg-background-primary px-4 py-3 text-left text-text-primary hover:border-accent/40 hover:bg-accent/5"
                    >
                      {label}
                    </button>
