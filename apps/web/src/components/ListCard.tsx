...
export function ListCard({ list, onDelete }: ListCardProps) {
  ...
  const handleShare = async () => {
    // TODO: Generate share link via Convex
        ...
        {showMenu && (
          <>
            ...
            <div className="absolute right-0 top-8 w-40 bg-background-secondary border border-white/10 rounded-xl shadow-xl z-20 overflow-hidden">
              <button
                onClick={handleShare}
                className="w-full flex items-center gap-2 px-4 py-2.5 text-left text-text-primary hover:bg-white/5 transition-colors"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
